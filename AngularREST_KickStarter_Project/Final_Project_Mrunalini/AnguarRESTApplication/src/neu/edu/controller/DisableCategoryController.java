...
@Controller
@Path("/user/{id}/disableCategory/{categoryId}")
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
public class DisableCategoryController {
...
@POST
public Response updateCategory(UserCategoryBean userCategoryBean,@PathParam("id") String id, @PathParam("categoryId") Integer categoryId) {
	
	String status = categoryService.updateCategory(userCategoryBean,id, categoryId);
	
	if (status == null || !status.equals("success")) {
		
		
		ResponseError categoryResponseErr = new ResponseError();
		categoryResponseErr.setMessage("Update failed");
		...
		ResponseError categoryResponseErr = new ResponseError();
		categoryResponseErr.setMessage("Update successful");
